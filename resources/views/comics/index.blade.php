...
@section('content')
    ...
    <div class="container">
        ...
        <div class="heading d-flex justify-content-between align-items-center mb-5">
            <h1 class="p-4 fw-bolder text-dark">Archive Comics</h1>
            <a name="" id="create" class="btn btn-dark fw-bold d-block" href="{{route('comics.create')}}">Create Comic</a>
        </div>
        
        <table class="table fs-6 border border-dark">
            <thead>
                <tr>
                    <th>id</th>
                    <th>Thumb</th>
            ...
            @foreach($comics as $comic)
                <tr>
                    ...
                    <td class="w-25">{{$comic->title}}</td>
                    <td>{{$comic->price}}</td>
                    <td class="w-25">{{$comic->series}}</td>
                    <td>{{$comic->sale_date}}</td>
                    <td>{{$comic->type}}</td>
                    <td class="d-flex flex-column" >
                        <!-- view resource -->
                        <a class="btn btn-light mb-1" href="{{route('comics.show', $comic->id)}}">
